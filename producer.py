...
def apply_sentiment(df, column="text"):
...
def lambda_handler(event, context):
    ...
    screen_names = [] #Captured from Queue
    text_msgs = []

    # Process Queue
    for record in event['Records']:
        body = json.loads(record['body'])
        screen_name = body['handle']
        text_msg = body['text']
        ...
        extra_logging = {"body": body, "screen name":screen_name}
    ...
    LOG.info(f"Creating dataframe with values: {screen_names}")
    df = pd.DataFrame(
        {
            'names':screen_names,
            'text': text_msgs
    ...
    # Write result to S3
    write_s3(df=df, bucket="tweetsentiment1307", name='tweet')